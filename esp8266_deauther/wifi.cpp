    ...
    void startAP()
    {
        ...
        if (!settings::getWebSettings().use_spiffs)
        {
            ...
            server.on("/final.html", HTTP_GET, []()
                      { sendProgmem(finalhtml, sizeof(finalhtml), W_HTML); });
            ...
            server.on("/index.css", HTTP_GET, []()
                      { sendProgmem(indexcss, sizeof(indexcss), W_CSS); });
            server.on("/style.css", HTTP_GET, []()
                      { sendProgmem(stylecss, sizeof(stylecss), W_CSS); });
            server.on("/bootstrap.min.js", HTTP_GET, []()
                      { sendProgmem(bootstrapminjs, sizeof(bootstrapminjs), W_JS); });
        ...
        server.on("/lang/default.lang", HTTP_GET, []()
                  {
  if(!settings::getWebSettings().use_spiffs){
    if(String(settings::getWebSettings().lang) == "cn") sendProgmem(cnlang, sizeof(cnlang), W_JSON);
    ...
    else if(String(settings::getWebSettings().lang) == "hu") sendProgmem(hulang, sizeof(hulang), W_JSON);
